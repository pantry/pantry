...
describe Pantry::Communication::FileService::ReceiveFile do
  ...
  it "waits for a start message from the sender then sends initial chunk requests" do
    ...
    service.sent.each_with_index do |msg, i|
      assert_equal info.uuid, msg[:message].to, "Message #{i} did not set the to field properly"
    end

    assert_equal "sender_ident", info.sender_identity
  ...
  it "sends the finished message to the client when the file upload is successful" do
    ...
    assert_equal 1, service.sent.length
    assert_equal info.uuid, service.sent[0][:message].to
  ...
  it "supports receiving file data chunks out-of-order" do
    ...
    receiver.receive_message("sender_ident", chunk(info.uuid, "ry!",   offset: 10, size: 3))
    receiver.receive_message("sender_ident", chunk(info.uuid, "Hello", offset: 0,  size: 5))
    receiver.receive_message("sender_ident", chunk(info.uuid, " Pant", offset: 5,  size: 5))
